...
public class ProductoService implements IProductoService {
	...
	public ResponseEntity<?> productSinEntity() {
	  ...
	  private List<VentaDto> findVentasConProductos() {
	        List<Object[]> resultados = dao.findVentasAndProductos();
	        List<VentaDto> ventasDTO = new ArrayList<>();
	        for (Object[] resultado : resultados) {
	            Integer idVenta = (Integer) resultado[0];
	            String nombreProducto = (String) resultado[1];
	            VentaDto ventaDto = new VentaDto(idVenta, nombreProducto);
